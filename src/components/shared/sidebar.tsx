...
SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        `fixed left-0 right-0 top-0 z-50 w-full border-r-2 border-r-muted transition-[width] md:relative md:bottom-0 md:right-auto md:h-svh ${
      ...
      <Layout>
        ...
        <LayoutHeader className="sticky top-0 justify-between px-4 py-3 shadow md:px-4">
          <div className={`flex items-center ${!isCollapsed ? "gap-2" : ""}`}>
            ...
            >
              <span className="font-medium">Agent Administration</span>
        ...
        <div
          className={cn(
            "h-full flex-1 overflow-auto group border-b bg-background py-2 transition-[max-height,padding] duration-500  md:border-none",
            navOpened ? "max-h-screen" : "max-h-0 py-0 md:max-h-screen md:py-2"
          )}
        >
          <nav className="grid gap-2 px-1">
            {SideLinksList.map((sidelink) => {
              const link = SideLinks[sidelink];
              return (
                <Link
                  key={link.id}
                  href={link.href}
